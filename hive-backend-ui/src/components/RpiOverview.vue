...
const memoryUsage = computed(() => {
  ...
  return (
    ((systemInfo.value.memoryTotal - systemInfo.value.memoryFree) /
      systemInfo.value.memoryTotal) *
    100
  );
});
...
const diskUsage = computed(() => {
  ...
  return (
    ((systemInfo.value.diskTotal - systemInfo.value.diskFree) /
      systemInfo.value.diskTotal) *
    100
  );
});
              ...
              >
                <template v-slot:default>
                  <strong>
                    {{
                      Math.round(
                        (systemInfo!.memoryTotal - systemInfo!.memoryFree) *
                          1000,
                      ) / 1000
                    }}
              ...
              >
                <template v-slot:default>
                  <strong
                    >{{
                      Math.round(
                        (systemInfo!.diskTotal - systemInfo!.diskFree) * 1000,
                      ) / 1000
                    }}
                    / {{ systemInfo!.diskTotal }} GB</strong
                  >
                </template>
              </v-progress-linear>
            </div>
