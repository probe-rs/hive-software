...
use comm_types::{cbor::CBOR_MIME, ipc::IpcMessage, results::TestResult};
...
pub(crate) fn post_test_results(results: Vec<TestResult>) -> (Request<Body>, Option<Vec<u8>>) {
    let mut bytes: Vec<u8> = vec![];
    into_writer(&IpcMessage::TestResults(results), &mut bytes)
